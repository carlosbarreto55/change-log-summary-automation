...
class ReportLabPDFExporter:
    """Render a domain release document and atomically replace its destination."""

    def export(self, document: ReleaseDocument, output_path: Path) -> Path:
        return export_release_pdf(document, output_path)

    def build_story(self, document: ReleaseDocument) -> List[Flowable]:
        return build_pdf_story(document)

...
class AnalysisPaths:
    """Canonical, validated destinations for one repository analysis."""

    repository_root: Path
    temp_diff_dir: Path
    output_path: Path
    _configured_temp_diff_dir: Path = field(repr=False, compare=False)
    _configured_output_path: Path = field(repr=False, compare=False)
    _protect_output: bool = field(repr=False, compare=False)

    def revalidate(self) -> AnalysisPaths:
        """Repeat containment checks after external directories are created."""
        temp_diff_dir, output_path = _validate_destinations(
            self.repository_root,
            self._configured_temp_diff_dir,
            self._configured_output_path,
            self._protect_output,
        )
        if temp_diff_dir != self.temp_diff_dir or output_path != self.output_path:
            raise RepositorySafetyError(
                "Analysis path identity changed after initial validation."
            )
        return self

    def prepare(self) -> AnalysisPaths:
        """Create external destination directories and revalidate their identities."""
        try:
            self.temp_diff_dir.mkdir(parents=True, exist_ok=True)
...
def validate_analysis_paths(
    ...
    return AnalysisPaths(
        ...
        _configured_temp_diff_dir=configured_temp_diff_dir,
        _configured_output_path=configured_output_path,
        _protect_output=protect_output,
    )

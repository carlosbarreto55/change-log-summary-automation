...
from release_notes_generator.infrastructure.openai import (
    OpenAIChatClient,
    SummaryClientFactoryAdapter,
)
...
class _UnusedFactory:
    def create(self, settings, env_file):
        raise AssertionError("explicit client must bypass the factory")


def summarize_diff_files(diff_files, client, max_characters_per_request):
    return summarize_diff_files_with_provenance(
...
def summarize_diff_files_with_provenance(
    ...
    artifacts = tuple(
        DiffArtifact(module_name, Path(path))
        for module_name, path in diff_files.items()
    )
    settings = ClaudeCodeAISettings(
        model="test",
        prompt="test",
        max_diff_characters_per_request=max_characters_per_request,
    )
    return SummarizationService(
        LocalArtifactStore(), _UnusedFactory(), client
    ).summarize(artifacts, settings, None)


def create_summary_client(settings, environ=None, env_file=None):
    return SummaryClientFactoryAdapter(environ=environ).create(settings, env_file)
...
class SummarizationTests(unittest.TestCase):
    ...
    def test_openai_chat_client_sends_expected_request_and_returns_summary(self) -> None:
        config = OpenAICompatibleAISettings(
        ...
        with patch("release_notes_generator.infrastructure.openai.urllib.request.urlopen") as urlopen:
    ...
    def test_openai_chat_client_requires_api_key_from_environment(self) -> None:
        ...
        with self.assertRaises(AISummarizationError):
            create_summary_client(config, environ={})
    ...
    def test_openai_chat_client_can_load_api_key_from_env_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            client = create_summary_client(config, environ={}, env_file=env_file)
...
class BackendFactoryTests(unittest.TestCase):
    def test_claude_code_configuration_builds_only_the_claude_client(self) -> None:
        ...
        with patch(
            "release_notes_generator.infrastructure.openai.OpenAIChatClient"
        ) as openai_client:
    ...
    def test_openai_configuration_builds_only_the_openai_client(self) -> None:
        ...
        with patch(
            "release_notes_generator.infrastructure.openai.ClaudeCodeClient"
        ) as claude_client:
            client = create_summary_client(
                config,
                environ={"CHANGE_LOG_SUMMARY_AI_API_KEY": "test-api-key"},

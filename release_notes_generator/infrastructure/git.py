"""Shell-free Git adapter for repository analysis and diff retrieval."""
...
class GitAdapter:
    """Implement repository-analysis and diff ports with the Git executable."""

    def inspect(
        self, repository_path: Path, analysis_head_ref: str
    ) -> RepositoryStatus:
        return inspect_repository(repository_path, analysis_head_ref)
    ...
    def update(
        ...
        return update_repository(
            repository_path,
            configuration.repository_update_mode.value,
            status=status,
            remote=configuration.refresh_remote,
            refspecs=configuration.refresh_refspecs,
        )
    ...
    def resolve_release_range(
        ...
        *,
        ...
        return _GitCommitExtractor(repository_path).resolve_release_range(
            head_ref, base_ref=base_ref, release_marker=release_marker
        )

    def commits_in_range(
        self, repository_path: Path, release_range: ReleaseRange
    ) -> tuple[Commit, ...]:
        return _GitCommitExtractor(repository_path).commits_in_range(release_range)

    def show(self, repository_path: Path, commit_hash: str) -> str:
        result = _run_read_only_git_process(
            repository_path,
            ["show", "--no-ext-diff", "--no-textconv", commit_hash, "--"],
        )
    ...
    if branch is None:
        ...
        if upstream_resolved:
            ...
            if comparison is None:
                ...
                if relation is not RepositoryRelation.EQUAL:
                    warnings.append(
                        f"Checkout branch '{branch}' is {relation.value} relative to upstream "
...
class _GitCommitExtractor:
    ...
    def commits_in_range(self, release_range: ReleaseRange) -> tuple[Commit, ...]:
...
def _classify_relation(ahead_count: int, behind_count: int) -> RepositoryRelation:
    if ahead_count and behind_count:
        return RepositoryRelation.DIVERGED
    if ahead_count:
        return RepositoryRelation.AHEAD
    if behind_count:
        return RepositoryRelation.BEHIND
    return RepositoryRelation.EQUAL
...
def _parse_commit_line(line: str) -> Commit:
    ...
    return Commit(commit_hash, author_email, subject, authored_at)

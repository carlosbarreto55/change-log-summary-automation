...
- **THEN** the commit is discarded before diff generation and AI processing
...
The system SHALL order modules according to their JSON order and SHALL order sections by the first appearance of each distinct section in that module order.

#### Scenario: Multiple modules share a section
- **WHEN** two included modules reference the same section
- **THEN** the release document contains one section heading with both module summaries in configured module order
...
- **THEN** the release document contains its title and a clear `No qualifying changes.` message
